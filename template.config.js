...
module.exports = {
  ...
  replaceFileTextFn: (fileText, templateName, utils) => {
    ...
    return fileText
      .replace(/__templateName__/g, templateName.toUpperCase())
      .replace(
        /__templateNameToPascalCase__/g,
        changeCase.pascalCase(templateName.toUpperCase()),
      )
      .replace(
        /__templateNameToParamCase__/g,
        changeCase.paramCase(templateName.toUpperCase()),
      );
  },
  renameFileFn: (fileName, templateName, utils) => {
    const {path} = utils;
    const {base} = path.parse(fileName);
    return base.replace(/__templateName__/gm, templateName.toUpperCase());
  },
  renameSubDirectoriesFn: (directoryName, templateName, _utils) => {
    const {changeCase} = _utils;
    const newDirectoryName = changeCase.paramCase(templateName.toUpperCase());
